...
const NewsCard = ({ id, urlimg, title, created_at, updated_at, user_id }) => {
  ...
  return (
    <Link
      className="group flex h-full flex-col rounded-xl border border-gray-200 p-5 transition-all duration-300 hover:border-transparent hover:shadow-lg dark:border-gray-700 dark:hover:border-transparent dark:hover:shadow-black/[.4]"
      href={`/news/${id}`}
    >
      ...
      <div className="mt-auto flex items-center gap-x-3">
        {user.picture && (
          <Image className="h-8 w-8 rounded-full" src={user.picture} alt="Image Description" />
        )}
        {!user.picture && (
          <Image className="h-8 w-8 rounded-full" src={defaultavatarimg} alt="Image Description" />
        )}
        <div>
